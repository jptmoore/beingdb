(** CLI: `clone` -- clone a remote Git repository into an Irmin Git store.
    Shared by the standalone [beingdb-clone] binary and the [beingdb
    clone] subcommand. *)
...
let dir_is_nonempty path =
  match Sys.readdir path |> Array.to_list with [] -> false | items -> List.length items > 0

let run_clone ~repo_url ~git_path ~force =
  let* () = Lwt_io.printl "BeingDB Clone" in
  ...
  let* () =
    if Sys.file_exists git_path then
      if not (Sys.is_directory git_path) then (
        let* () = Lwt_io.eprintlf "Error: --git path exists and is not a directory: %s" git_path in
        Lwt.fail (Failure "Invalid --git path"))
      else if dir_is_nonempty git_path && not force then (
        let* () = Lwt_io.eprintlf "Error: --git directory already exists and is not empty: %s" git_path in
        let* () = Lwt_io.eprintl "Refusing to clone into an existing store. Use --force to reuse it." in
        Lwt.fail (Failure "Refusing to clone into non-empty directory"))
      else if dir_is_nonempty git_path && force then
        Lwt_io.eprintl "Warning: --force set; reusing existing --git directory (HEAD may change)"
  ...
  Lwt.catch
    ...
    (fun exn ->
      ...
      if is_network_error then (
        ...
        let* () = Lwt_io.eprintlf "   beingdb-import --input <cloned-dir> --git %s" git_path in
        ...
        let* () = Lwt_io.eprintl "- For private repos, use token: https://TOKEN@github.com/user/repo.git" in
        let* () = Lwt_io.eprintl "- Check network connectivity" in
        let* () = Lwt_io.eprintl "" in
        let* () = Lwt_io.eprintl "Note: SSH URLs (git@github.com:...) are not currently supported." in
...
let clone_repo repo_url git_path force =
  ...
  | Ok () -> ()
  | Error exn ->
      (* Avoid Cmdliner backtraces: errors are already printed *)
      (match exn with Failure _ -> () | _ -> Printf.eprintf "Error: %s\n" (Printexc.to_string exn));
      exit 1
