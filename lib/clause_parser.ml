(** Clause_parser: parses a single token list into one {!Query_ast.clause}
    (a predicate pattern, comparison, or between-clause), and splits a
    token list on top-level commas. Shared by {!Query_parser} (the core
    query language) and {!Dsl_parser} (the expressive query language) so
    both accept exactly the same literal/clause syntax. *)

let term_of_token = function
  | Lexer.Underscore -> Ok Query_ast.Wildcard
  | Lexer.Ident s when s <> "" && Char.uppercase_ascii s.[0] = s.[0] && s <> "true" && s <> "false" ->
      Ok (Query_ast.Variable s)
  | tok -> ( match Lexer.value_of_token tok with Ok v -> Ok (Query_ast.Literal v) | Error e -> Error e)
...
let parse_clause_tokens tokens =
  ...
  | Lexer.Ident predicate :: Lexer.LParen :: rest when List.rev rest <> [] && List.hd (List.rev rest) = Lexer.RParen -> (
      ...
      | Ok arguments -> Ok (Query_ast.Pattern { predicate; arguments })
      | Error e -> Error e)
  | [ left_tok; Lexer.Ident "between"; lower_tok; Lexer.Ident "and"; upper_tok ] -> (
      ...
      | Ok value, Ok lower, Ok upper -> Ok (Query_ast.Between { value; lower; upper })
  ...
  | [ left_tok; Lexer.Op op; right_tok ] -> (
      ...
      | Some operator -> (
          ...
          | Ok left, Ok right -> Ok (Query_ast.Compare { left; operator; right })
