...
(** Non-blank, non-comment lines, each tagged with its 1-based source
    ...
    (within the original, untrimmed line), and trimmed text. *)
let significant_lines text =
  ...
  |> List.mapi (fun i raw ->
         ...
         let col =
           ...
           let rec first_non_ws j = if j >= len then 1 else if raw.[j] = ' ' || raw.[j] = '\t' then first_non_ws (j + 1) else j + 1 in
           first_non_ws 0
         in
         (i + 1, col, trimmed))
  |> List.filter (fun (_, _, line) -> line <> "" && not (starts_with ~prefix:"%" line) && not (starts_with ~prefix:"#" line))
...
let is_group_boundary (_, _, line) =
...
let parse_clause_line (line_no, column, text) =
  ...
  in
  match clause with
  | Query_ast.Pattern { predicate; arguments } -> Ok (Surface_ast.Pattern { predicate; arguments; line = line_no; column })
  | Query_ast.Compare { left; operator; right } -> Ok (Surface_ast.Compare { left; operator; right; line = line_no; column })
  | Query_ast.Between { value; lower; upper } -> Ok (Surface_ast.Between { value; lower; upper; line = line_no; column })
...
let rec parse_clauses lines =
  match lines with
  | [] -> Ok ([], [])
  | (_, _, line) :: _ when is_tail_keyword line -> Ok ([], lines)
  | (_, _, "optional") :: rest ->
  ...
  | (line_no, _, "or") :: _ -> Error (Printf.sprintf "Line %d: 'or' without a preceding 'either'" line_no)
  | line :: rest ->
...
let rec parse_tail lines order_acc limit_acc offset_acc =
  ...
  | (line_no, _, line) :: rest when line = "order by" || starts_with ~prefix:"order by " line ->
      let trailing = if line = "order by" then "" else drop_prefix "order by " line in
      let item_lines, remaining = take_while (fun (_, _, l) -> not (is_tail_keyword l)) rest in
      let raw = (if trailing = "" then [] else [ (line_no, trailing) ]) @ List.map (fun (ln, _, text) -> (ln, text)) item_lines in
      let flattened =
  ...
  | (line_no, _, line) :: rest when line = "limit" || starts_with ~prefix:"limit " line -> (
  ...
  | (line_no, _, line) :: rest when line = "offset" || starts_with ~prefix:"offset " line -> (
  ...
  | (line_no, _, line) :: _ -> Error (Printf.sprintf "Line %d: unexpected content: %s" line_no line)
...
let parse text =
  ...
  | (_, _, first) :: rest when first = "find" || starts_with ~prefix:"find " first ->
      ...
      | (line_no, _, _) :: _ -> Error (Printf.sprintf "Line %d: expected 'where' after 'find ...'" line_no)
      | [] -> Error "Expected 'where' after 'find ...'")
  | (line_no, _, _) :: _ -> Error (Printf.sprintf "Line %d: expected query to start with 'find ...'" line_no)
