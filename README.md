...
Facts can therefore be edited using normal development workflows including commits, branches, pull requests, review and version history.

`beingdb-compile` transforms those source facts into an optimized store.

`beingdb-serve` opens that compiled store read-only and exposes it through the query API.

This keeps the runtime simple and makes deployments reproducible.

## Example

A small cultural heritage dataset might contain:

```text
artist(tina_keane).
work(shadow_of_a_journey).
venue(ica_london).
...
BeingDB can directly query relationships:
...
This follows the factual chain:

```text
Artist -> Work -> Exhibition -> Venue
```

without requiring those relationships to be reconstructed from unstructured text.
